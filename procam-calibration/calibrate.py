...
def main():
    ...
    parser.add_argument('-data_folder', type=str, 
                        default='/home/larry/codeGit/evt_3d_project/procam-calibration/sample_data/capture_0',help='calibration data folder')
    ...
    dirnames = sorted(glob.glob(data_folder))
    if len(dirnames) == 0:  
        print('Directories ', data_folder, ' were not found')
        return
    ...
    calibrate(used_dirnames, gc_fname_lists, proj_shape, chess_shape, chess_block_size, gc_step, black_thr, white_thr, camP, cam_dist)
...
def calibrate(dirnames, gc_fname_lists, proj_shape, chess_shape, chess_block_size, gc_step, black_thr, white_thr, camP, camD):
    ...
    objps[:, :2] = chess_block_size * np.mgrid[0:chess_shape[0], 0:chess_shape[1]].T.reshape(-1, 2)     # corner in projector
    ...
    for dname, gc_filenames in zip(dirnames, gc_fname_lists):
        ...
        for fname in gc_filenames:
            ...
            if cam_shape != img.shape:
                ...
                return None
            imgs.append(img)
        black_img = imgs.pop()      # the last image is black-image (projector is off, dark)
        white_img = imgs.pop()      # the last-2 image is white (projector is on)

        res, cam_corners = cv2.findChessboardCorners(white_img, chess_shape)

        
        # debug_white_img = cv2.cvtColor(white_img, cv2.COLOR_GRAY2RGB)
        # # debug_cam_corners = np.array(cam_corners)
        # color_idx = 0
        # for point in cam_corners:
        #     center = (int(point[0][0]), int(point[0][1]))
        #     cv2.circle(debug_white_img, center, radius=10, color=(0, 255, color_idx), thickness=-1)
        #     color_idx = color_idx + 10
        # cv2.namedWindow("white_img", cv2.WINDOW_FREERATIO)
        # cv2.imshow("white_img", debug_white_img)
        cv2.waitKey(0)
        print("Index: ", idx)
        idx = idx + 1
        ...
        for corner, objp in zip(cam_corners, objps):            # For one white-black pair,
            ...
            for dx in range(-patch_size_half, patch_size_half + 1):
                for dy in range(-patch_size_half, patch_size_half + 1):
                    x = c_x + dx
                    y = c_y + dy
                    if int(white_img[y, x]) - int(black_img[y, x]) <= black_thr:        # TODO: need a abs()?
                        continue
                    err, proj_pix = graycode.getProjPixel(imgs, x, y)               # Project point in camera into projector. But Not know the projection matrix now. 
                    if not err:
            ...
            #     cv2.circle(debug_white_img, center, radius=10, color=(0, 0, 255), thickness=2)
            # for pt in src_points:
            #     center = (int(pt[0]), int(pt[1]))
            #     cv2.circle(debug_white_img, center, radius=10, color=(0, 255, 0), thickness=2)
            # cv2.imshow("compare", debug_white_img)
            # cv2.waitKey(0)
            # ## Dongy Debug
    ...
    cam_rvecs = []
    cam_tvecs = []
    # 1. If camera not calibrated, first calib it.
    if(camP is None):
        ret, cam_int, cam_dist, cam_rvecs, cam_tvecs = cv2.calibrateCamera(cam_objps_list, cam_corners_list, cam_shape, None, None, None, None)
        print('  RMS :', ret)
    # 1.1 If camera calibrated, calculate the pose (R,t) between camera & projected chessboard for each image.
    else:
    ...
    # 2. Calibrate the projection instrinsics
    ...
    print('=== Result ===')
    ret, cam_int, cam_dist, proj_int, proj_dist, cam_proj_rmat, cam_proj_tvec, E, F = cv2.stereoCalibrate(proj_objps_list, cam_corners_list2, proj_corners_list, cam_int, cam_dist, proj_int, proj_dist, None)
    print('  RMS :', ret)
